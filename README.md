...
HoYoLAB 원신, 붕괴: 스타레일, 젠레스 존 제로 전적 페이지에서 캐릭터 정보를 읽어, 커뮤니티 기준표 **"이정도면 잘키웠다"** 와 비교하는 브라우저 확장 프로그램.

> HoYoverse 공식 프로젝트가 아닌 팬 메이드 도구.
...
Chrome 사용자는 아래 링크에서 바로 설치 가능.

https://chromewebstore.google.com/detail/ilgdadofigikokamdfgamikpdgbhkgeo?utm_source=item-share-cb

**개발자 모드 수동 설치**

Chrome Web Store를 사용할 수 없거나 로컬 버전을 테스트할 때는 개발자 모드로 직접 불러오기 가능.

**Chrome / Edge 공통**

1. 이 저장소 다운로드 또는 압축 해제.
2. 브라우저 확장 프로그램 관리 페이지 열기.
   - Chrome: `chrome://extensions`
   - Edge: `edge://extensions`
3. 오른쪽 위의 **개발자 모드** 활성화.
4. **압축해제된 확장 프로그램을 로드** 클릭.
5. `manifest.json`이 있는 저장소 루트 폴더 선택.
6. 목록에 `jalkiwotda-hoyoverse`가 표시되면 설치 완료.

---

## 사용할 사이트

아래 링크를 브라우저 즐겨찾기에 추가한 뒤, 즐겨찾기에서 바로 열어 사용하는 것을 권장. HoYoLAB 페이지 구조가 복잡해 일반 메뉴에서 다른 페이지로 들어가면 확장 프로그램 패널이 보이지 않을 수 있음.
...
1. 확장 프로그램을 설치한 브라우저에서 HoYoLAB 로그인.
2. HoYoLAB 언어를 **한국어**로 설정.
3. 위 표의 게임별 HoYoLAB 링크를 즐겨찾기하고, 즐겨찾기에서 원하는 게임 전적 페이지 열기.
4. 페이지 새로고침.
5. 화면 오른쪽 아래 패널 확인.
6. HoYoLAB이 캐릭터 정보를 로드하면 **불러온 정보** 숫자 증가.
7. **정오표 보기** 버튼 클릭 후 분석 리포트 확인.

> **참고** - HoYoLAB 언어가 한국어가 아니면 캐릭터 이름·스탯명이 매칭되지 않음. 이 경우 확장 프로그램이 언어 변경 안내 표시.

### 패널 버튼

| 버튼 | 설명 |
|------|------|
| 정오표 보기 | 현재 캡처된 캐릭터 정보를 기준표와 비교해 리포트 열기 |
| 새로고침 | 캡처 데이터를 초기화하고 HoYoLAB 페이지 새로고침 |
...
이 확장 프로그램이 사용하는 데이터는 다음과 같음.

- HoYoLAB 페이지가 브라우저에서 로드한 캐릭터 응답
- 공개 구글 스프레드시트 기준표
- 일부 게임의 HoYoWiki 장비 세트 이름 정보

로그인 토큰이나 비밀번호를 수집하지 않으며, 모든 분석은 사용자의 브라우저 안에서 처리됨. 외부 서버로 데이터 전송 없음.

브라우저 권한에는 기준표를 가져오기 위한 `docs.google.com` 접근 포함.

개인정보 처리방침은 [PRIVACY.md](PRIVACY.md) 참고. Chrome Web Store 제출용 메모와 권한 설명은 [CHROME_WEB_STORE.md](CHROME_WEB_STORE.md)에 정리.
...
**패널이 보이지 않을 때**
- 위의 게임별 HoYoLAB 링크를 즐겨찾기하고, 해당 즐겨찾기 링크로 다시 접속.
- 확장 프로그램 설치 후 페이지 새로고침.
- `chrome://extensions`에서 확장 프로그램 활성화 여부 확인.

**"불러온 정보"가 계속 0일 때**
- HoYoLAB 로그인 여부 확인.
- 캐릭터 정보가 완전히 로드될 때까지 기다린 후 다시 확인.
- 페이지 새로고침 후 재시도.

**리포트가 뜨지 않고 "아직 캐릭터 정보를 가져오지 못했습니다"가 표시될 때**
- HoYoLAB 페이지를 새로고침한 뒤 캐릭터 정보가 완전히 로드될 때까지 기다렸다가 **정오표 보기** 다시 클릭.

**기준표와 매칭이 안 될 때**
- HoYoLAB 언어가 한국어인지 확인.
- 여행자, 개척자처럼 속성이나 운명의 길에 따라 이름이 달라지는 캐릭터는 자동 매칭 제한 가능.
- HoYoLAB 데이터 구조나 기준표 표기가 변경된 경우 일부 항목이 `매칭 없음` 또는 `?`로 표시될 수 있음.

---

## 현재 한계

- HoYoLAB 페이지 구조나 API 응답 형식이 변경되면 동작하지 않을 수 있음.
- 기준표의 셀 구조가 크게 바뀌면 파싱 실패 가능.
- 자동 비교는 기준표 문구와 HoYoLAB 표기를 정규화하여 매칭하는 방식으로, 모든 케이스를 완벽하게 판단하지는 못함.
- 현재 리포트 UI는 검증 목적의 간단한 HTML 테이블 형태.
...
각 게임 코드는 기존 단독 확장의 namespace와 message type 유지.
