...
Chrome 사용자는 아래 링크에서 바로 설치할 수 있습니다.

https://chromewebstore.google.com/detail/ilgdadofigikokamdfgamikpdgbhkgeo?utm_source=item-share-cb

**개발자 모드 수동 설치**

Chrome Web Store를 사용할 수 없거나 로컬 버전을 테스트할 때는 개발자 모드로 직접 불러와서 사용할 수 있습니다.
...
6. 목록에 `jalkiwotda-hoyoverse`가 표시되면 설치 완료입니다.

---

## 사용할 사이트

아래 링크를 브라우저 즐겨찾기에 추가한 뒤, 즐겨찾기에서 바로 열어 사용하는 것을 권장합니다. HoYoLAB 페이지 구조가 복잡해 일반 메뉴에서 다른 페이지로 들어가면 확장 프로그램 패널이 보이지 않을 수 있습니다.
...
기준표 원본을 확인해야 할 때는 아래 구글 스프레드시트를 참고하세요.
...
3. 위 표의 게임별 HoYoLAB 링크를 즐겨찾기하고, 즐겨찾기에서 원하는 게임 전적 페이지를 엽니다.
4. 페이지를 새로고침합니다.
...
브라우저 권한에는 기준표를 가져오기 위한 `docs.google.com` 접근이 포함됩니다.

개인정보 처리방침은 [PRIVACY.md](PRIVACY.md)를 참고하세요. Chrome Web Store 제출용 메모와 권한 설명은 [CHROME_WEB_STORE.md](CHROME_WEB_STORE.md)에 정리되어 있습니다.
...
**패널이 보이지 않을 때**
- 위의 게임별 HoYoLAB 링크를 즐겨찾기하고, 해당 즐겨찾기 링크로 다시 접속하세요.
...
**기준표와 매칭이 안 될 때**
- HoYoLAB 언어가 한국어인지 확인하세요.
- 여행자, 개척자처럼 속성이나 운명의 길에 따라 이름이 달라지는 캐릭터는 자동 매칭이 제한될 수 있습니다.
...
각 게임 코드는 기존 단독 확장의 namespace와 message type을 유지합니다.
