(() => {
    ...
    for (const row of rows.slice(headerIndex + 2)) {
      ...
      const variants = fillMergedSheetCells(blockRows)
        ...
        .map((blockRow) => ({
          ...
          eidolons: formatEidolonCell(blockRow[12]),
  ...
  function requestSheetFromExtension() {
    return new Promise((resolve, reject) => {
      ...
      function handleMessage(event) {
        ...
        if (event.data.ok) {
          console.info(`[jalkiwotda-hsr] sheet source: ${event.data.source || "unknown"}`);
  ...
  async function loadSheet() {
    try {
      ...
      console.warn("[jalkiwotda-hsr] extension sheet bridge failed", error);
      throw error;
    }
  }
  ...
  function debugCharacterRows(characterName) {
    ...
    if (endIndex < 0) {
      console.warn("[jalkiwotda-hsr] character sheet rows not found", characterName);
      return [];
    }
    ...
    console.log(`[jalkiwotda-hsr] ${characterName} raw sheet rows`, result);
    console.log(`[jalkiwotda-hsr] ${characterName} sheet merges`, merges);
